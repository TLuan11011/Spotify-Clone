...
from .models import Song, Playlist, PlaylistSong, Album, Artist, User
...
def get_playlists(request):
    ...
    if not user_id:
        return Response([], status=status.HTTP_200_OK)  # Trả về danh sách rỗng nếu không có user_id
...
def login_user(request):
    ...
    if password != user.password_hash:
        return Response({'error': 'Sai email hoặc mật khẩu'}, status=status.HTTP_401_UNAUTHORIZED)
    if hasattr(user, 'trang_thai') and user.status != 1:
